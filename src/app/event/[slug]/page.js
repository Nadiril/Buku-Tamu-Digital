  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubmittedData({ ...form, status_kehadiran: data.status_kehadiran });
        setSubmitted(true);
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden">
        ...
        <div className="relative text-center max-w-md mx-4">
          ...
          <p className="text-muted text-sm mb-2">Data kehadiran Anda telah berhasil tercatat untuk acara:</p>
          ...
          <Button variant="secondary" onClick={() => { setSubmitted(false); setForm({ nama: "", instansi: "", tujuan: "", no_hp: "" }); setSubmittedData(null); }}>
            Daftar Tamu Lain
          </Button>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-lg mx-4">
        ...
        <div className="glass-card rounded-2xl p-8 glow-accent">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Input id="guest-instansi" label="Instansi / Lembaga" placeholder="Contoh: Universitas Airlangga" value={form.instansi} onChange={(e) => setForm({ ...form, instansi: e.target.value })} required icon={
        ...
        <p className="text-center text-xs text-muted/50 mt-6">Buku Tamu Digital — Powered by Next.js</p>
      </div>
    </div>
  );
}
